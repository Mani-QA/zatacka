...
export interface RtcPollResponse {
  peers: PeerRow[];
  signals: SignalRow[];
  /** Server-elected host — first occupant of the room. */
  hostId?: string;
}
...
export interface PeerInfo {
  ...
  rttMs: number | null;
  /** True once the reliable data channel can send handshake/lobby messages. */
  reliableReady: boolean;
}
...
export interface P2PRoomOptions {
  ...
  onConnected?: () => void;
  /** Server-authoritative host id, sent on every successful poll. */
  onHostId?: (hostId: string) => void;
  /** Transient signaling/HTTP failures. The poll loop keeps retrying. */
...
export class P2PRoom {
  ...
  private async pollOnce(): Promise<void> {
    ...
    if (!ctype.includes("application/json")) throw new Error("signaling poll failed: non-json");
    const body = (await res.json()) as RtcPollResponse;
    if (this.closed) return;
    this.reconcileRoster(body.peers);
    if (typeof body.hostId === "string" && body.hostId) {
      this.announcedHost = body.hostId;
      this.opts.onHostId?.(body.hostId);
    }
    if (!this.everPolled) {
  ...
  private attachChannel(slot: PeerSlot, channel: RTCDataChannel): void {
    ...
    const markReliable = () => {
      ...
      slot.info.reliableReady = channel.readyState === "open";
      this.emitPeers();
    };
    channel.onopen = () => {
      slot.lastProgressAt = Date.now();
    ...
    if (channel.readyState === "open") markReliable();
    channel.onmessage = (e) => {
  ...
  private emitPeers(): void {
    ...
    const fingerprint = JSON.stringify(
      list.map((p) => [p.id, p.name, p.connectionState, p.candidateType, p.rttMs, p.reliableReady]),
    );
